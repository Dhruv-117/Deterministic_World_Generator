...
# Biome-aware elevation reshaping controls
ENABLE_BIOME_ELEVATION_RESHAPING = True
...
ELEVATION_LAND_OUT_MAX = 0.98
ELEVATION_LAND_GAMMA = 1.04

# River-stability tolerance checks after reshaping
RIVER_TOLERANCE_TOTAL_TILES = 0.55      # ±55%
RIVER_TOLERANCE_OCEAN_REACH = 0.45      # ±45%
...
BIOME_ELEVATION_PROFILES = {
    ...
    'sand_desert': {'level': 0.46, 'relief': 0.64},
    'rock_desert': {'level': 0.57, 'relief': 0.90},
    'badlands': {'level': 0.62, 'relief': 0.96},
    'oasis': {'level': 0.42, 'relief': 0.56},
    'snow_plains': {'level': 0.57, 'relief': 0.64},
    'snow_forest': {'level': 0.60, 'relief': 0.74},
    'swamp': {'level': 0.38, 'relief': 0.46},
    'marsh': {'level': 0.40, 'relief': 0.48},
    'mangrove': {'level': 0.36, 'relief': 0.42},
    'lake': {'level': 0.34, 'relief': 0.30},
    'ocean': {'level': 0.10, 'relief': 0.20},
}
...
RIVER_FLOW_THRESHOLD = 10              # Min flow accumulation to form a river
RIVER_MAJOR_THRESHOLD = 50             # Flow threshold for major rivers
RIVER_MIN_HEADWATER_FLOW = 10          # Same as threshold - headwater is where river "starts"
...
TARGET_MAX_RIVERS = 28                 # Target number of main river systems (default)
MIN_HEADWATER_SPACING = 11             # Min tiles between river headwaters
TRIBUTARY_SPACING_FACTOR = 0.70        # Tributary spacing = MIN_HEADWATER_SPACING * this factor
MAX_TRIBUTARIES_PER_RIVER = 4          # Maximum tributaries per main river system
INLAND_RIVER_CHANCE = 0.34             # Chance to add additional inland rivers
INTERIOR_RIVER_DIST_THRESHOLD = 20     # Distance from ocean considered "continental interior"
MIN_INTERIOR_RIVER_COVERAGE = 0.011    # Min river tile coverage on interior land (1.1%)
MAX_INTERIOR_RIVER_ADDITIONS = 5       # Cap extra interior systems to avoid messy networks
...
class SeededNoiseGenerator:
    ...
    def _fallback_noise(self, x, y, scale, octaves, persistence):
        """Fallback noise generation using smooth interpolated value noise."""
        value = 0.0
        amplitude = 1.0
        max_value = 0.0
        frequency = 1.0 / scale
        ...
        for _ in range(octaves):
            ...
            hash_val = self._value_noise_2d(nx, ny)
    ...
    def _value_noise_2d(self, x, y):
        """Continuous 2D value noise sampled from hashed lattice corners."""
        x0 = math.floor(x)
        y0 = math.floor(y)
        x1 = x0 + 1
        y1 = y0 + 1
        ...
        v00 = self._hash_2d(x0, y0)
        v10 = self._hash_2d(x1, y0)
        v01 = self._hash_2d(x0, y1)
        v11 = self._hash_2d(x1, y1)

        ix0 = self._lerp(v00, v10, sx)
        ix1 = self._lerp(v01, v11, sx)
...
def classify_landforms(elevation, slope, is_land, is_mountain):
    ...
    # Foothill ring: smooth mountain-to-plains transitions by promoting
    # immediate mountain-adjacent plains to hills where plausible.
    if is_mountain.any():
        dist_to_mountain = ndimage.distance_transform_edt(~is_mountain)
        foothill_ring = (
            is_land
            & ~is_mountain
            & (dist_to_mountain <= 2.2)
            & (landforms == 'plains')
            & ((slope > slope_low * 0.75) | (elevation > elev_low * 0.92))
        )
        landforms[foothill_ring] = 'hills'
...
def summarize_elevation_stats(label, elevation, land_mask):
    """Print compact elevation diagnostics on land tiles."""
    if not land_mask.any():
        print(f"  - {label}: no land tiles")
        return

    land_vals = elevation[land_mask]
    p10, p50, p90 = np.percentile(land_vals, [10, 50, 90])
    print(
        f"  - {label}: min={land_vals.min():.3f}, p10={p10:.3f}, "
        f"p50={p50:.3f}, p90={p90:.3f}, max={land_vals.max():.3f}, std={land_vals.std():.3f}"
    )


def _safe_normalize(values, mask, low_pct=3, high_pct=97):
    """Percentile normalization on masked values to a stable [0,1] range."""
    out = np.zeros_like(values, dtype=np.float32)
    masked = values[mask]
    if masked.size == 0:
        return out

    low = float(np.percentile(masked, low_pct))
    high = float(np.percentile(masked, high_pct))
    span = max(1e-6, high - low)
    out[mask] = np.clip((values[mask] - low) / span, 0.0, 1.0)
    return out
...
def _build_biome_profile_maps(biomes, land_mask):
    ...
    height, width = biomes.shape
    target_level = np.zeros((height, width), dtype=np.float32)
    relief_scale = np.ones((height, width), dtype=np.float32)
    ...
    for y in range(height):
        for x in range(width):
            if not land_mask[y, x]:
                continue
            biome = biomes[y, x]
            profile = BIOME_ELEVATION_PROFILES.get(biome)
            if profile is None:
                category = BIOME_CATEGORIES.get(biome, 'plains')
                lvl, rel = default_by_category.get(category, (0.50, 0.80))
            else:
                lvl = profile['level']
                rel = profile['relief']
            target_level[y, x] = float(lvl)
            relief_scale[y, x] = float(rel)

    return target_level, relief_scale


def _mountain_mask_from_biomes(biomes, land_mask):
    """Build a boolean mask for mountain-category biome tiles on land."""
    mountain_mask = np.zeros(biomes.shape, dtype=bool)
    for biome_name, category in BIOME_CATEGORIES.items():
        if category == 'mountains':
            mountain_mask |= (biomes == biome_name)
    mountain_mask &= land_mask
    return mountain_mask


def _build_mountain_structure_delta(biomes, land_mask, seed):
    """Create deterministic ridge/valley/peak structure for mountain regions."""
    mountain_mask = _mountain_mask_from_biomes(biomes, land_mask)
    ...
    rng = np.random.default_rng(seed + 14221)

    labeled, num_features = ndimage.label(mountain_mask, structure=np.ones((3, 3), dtype=np.int32))
    ...
    for comp_id in range(1, num_features + 1):
        ...
        if area < MOUNTAIN_COMPONENT_MIN_TILES:
            continue

        ys, xs = np.where(comp_mask)
        cy = float(np.mean(ys))
        cx = float(np.mean(xs))
        size_scale = float(np.clip(np.sqrt(area) / 11.0, 0.75, 2.8))
        span_y = float(np.max(ys) - np.min(ys) + 1)
        span_x = float(np.max(xs) - np.min(xs) + 1)
        elongation = max(span_x, span_y) / max(1.0, min(span_x, span_y))
        ...
        if archetype == 'plateau_valley':
            ...
            ridge_count = int(3 + np.round(1.6 * size_scale))
            valley_count = int(2 + np.round(1.2 * size_scale))
            peak_count = int(1 + np.round(1.2 * size_scale))
            theta_jitter = 0.18
            peak_amp = 0.43
        elif archetype == 'ghat':
            ridge_count = int(3 + np.round(1.4 * size_scale))
            valley_count = int(3 + np.round(1.6 * size_scale))
            ...
            peak_count = int(2 + np.round(1.5 * size_scale))
            theta_jitter = 0.42
            peak_amp = 0.48

        component_field = np.zeros((height, width), dtype=np.float32)
        centered = np.stack([xs.astype(np.float32) - cx, ys.astype(np.float32) - cy], axis=1)
        if centered.shape[0] >= 3:
            cov = np.cov(centered, rowvar=False)
            eigvals, eigvecs = np.linalg.eigh(cov)
            principal = eigvecs[:, int(np.argmax(eigvals))]
            base_theta = float(np.mod(np.arctan2(principal[1], principal[0]), np.pi))
        else:
            base_theta = float(rng.uniform(0.0, np.pi))
        ...
        for _ in range(ridge_count):
            ...
            ry = float(ys[index])
            rx = float(xs[index])
            theta = base_theta + float(rng.normal(0.0, theta_jitter))

            dy = yy - ry
            dx = xx - rx
            along = np.cos(theta) * dx + np.sin(theta) * dy
            cross = -np.sin(theta) * dx + np.cos(theta) * dy

            ridge_length = (5.0 + 6.0 * size_scale) * float(0.75 + 0.70 * rng.random())
            ridge_width = (1.2 + 1.3 * size_scale) * float(0.70 + 0.50 * rng.random())
            ridge_amp = float(0.42 + 0.30 * rng.random())

            ridge_shape = np.exp(-(cross ** 2) / (2.0 * ridge_width ** 2)) * np.exp(-(along ** 2) / (2.0 * ridge_length ** 2))
            component_field += ridge_amp * ridge_shape

        for _ in range(valley_count):
            vy = float(cy + rng.normal(0.0, 2.5 * size_scale))
            vx = float(cx + rng.normal(0.0, 2.5 * size_scale))
            theta = base_theta + (np.pi / 2.0) + float(rng.normal(0.0, 0.50))
            ...
            valley_length = (3.5 + 4.5 * size_scale) * float(0.75 + 0.65 * rng.random())
            valley_width = (1.3 + 1.2 * size_scale) * float(0.80 + 0.35 * rng.random())
            valley_amp = -float(0.20 + 0.22 * rng.random())
            meander_scale = max(2.6, 3.0 + 1.1 * size_scale)
            meander_phase = float(rng.uniform(0.0, 2.0 * np.pi))
            meander = np.sin((along / meander_scale) + meander_phase) * (MOUNTAIN_VALLEY_MEANDER_STRENGTH * valley_width)
            cross = cross + meander

            valley_shape = np.exp(-(cross ** 2) / (2.0 * valley_width ** 2)) * np.exp(-(along ** 2) / (2.0 * valley_length ** 2))
            component_field += valley_amp * valley_shape

        for _ in range(peak_count):
            index = int(rng.integers(0, len(ys)))
            py = float(ys[index])
            px = float(xs[index])
            radius = float(1.0 + (2.0 + 1.4 * size_scale) * rng.random())
            amp = float(peak_amp + 0.34 * rng.random())

            dist2 = (yy - py) ** 2 + (xx - px) ** 2
            peak_shape = np.exp(-dist2 / (2.0 * radius ** 2))
            component_field += amp * peak_shape

        if archetype == 'ghat':
            stair_sigma = float(max(1.2, 1.8 * size_scale))
        ...
        if area > 220:
            mega_theta = base_theta + float(rng.normal(0.0, 0.18))
            dy = yy - cy
            dx = xx - cx
            along = np.cos(mega_theta) * dx + np.sin(mega_theta) * dy
            cross = -np.sin(mega_theta) * dx + np.cos(mega_theta) * dy
            mega_length = (11.0 + 12.0 * size_scale) * float(0.85 + 0.35 * rng.random())
            mega_width = (2.4 + 1.8 * size_scale) * float(0.80 + 0.30 * rng.random())
            mega_spine = np.exp(-(cross ** 2) / (2.0 * mega_width ** 2)) * np.exp(-(along ** 2) / (2.0 * mega_length ** 2))

            if area > MOUNTAIN_MEGA_SYSTEM_THRESHOLD:
                branch_count = int(np.clip(np.round(2.0 + size_scale), 2, 6))
                branch_field = np.zeros((height, width), dtype=np.float32)
                for _ in range(branch_count):
                    btheta = mega_theta + float(rng.normal(0.0, 0.42))
                    by = float(cy + rng.normal(0.0, 3.5 * size_scale))
                    bx = float(cx + rng.normal(0.0, 3.5 * size_scale))
                    bdy = yy - by
                    bdx = xx - bx
                    balong = np.cos(btheta) * bdx + np.sin(btheta) * bdy
                    bcross = -np.sin(btheta) * bdx + np.cos(btheta) * bdy
                    blen = (5.8 + 5.4 * size_scale) * float(0.80 + 0.50 * rng.random())
                    bwid = (1.2 + 1.2 * size_scale) * float(0.75 + 0.45 * rng.random())
                    bamp = float(0.20 + 0.22 * rng.random())
                    branch_shape = np.exp(-(bcross ** 2) / (2.0 * bwid ** 2)) * np.exp(-(balong ** 2) / (2.0 * blen ** 2))
                    branch_field += bamp * branch_shape
                component_field += branch_field * MOUNTAIN_BRANCH_CHAIN_STRENGTH

        small_chain = np.zeros((height, width), dtype=np.float32)
        micro_count = int(2 + np.clip(np.round(size_scale * 2.4), 1, 8))
        for _ in range(micro_count):
            index = int(rng.integers(0, len(ys)))
            py = float(ys[index])
            px = float(xs[index])
            radius = float(0.8 + 1.2 * rng.random())
            amp = float(0.18 + 0.20 * rng.random())
            dist2 = (yy - py) ** 2 + (xx - px) ** 2
            small_chain += amp * np.exp(-dist2 / (2.0 * radius ** 2))
        ...
        core = ndimage.gaussian_filter(comp_mask.astype(np.float32), sigma=1.4 + 0.4 * size_scale)
        foothill = ndimage.gaussian_filter(comp_mask.astype(np.float32), sigma=3.6 + 1.0 * size_scale)
        influence = np.clip(0.86 * core + 0.52 * foothill, 0.0, 1.0)

        local_support = (foothill > 0.03) & land_mask
        local_vals = component_field[local_support]
        if local_vals.size > 0:
            norm = float(np.percentile(np.abs(local_vals), 94))
            if norm > 1e-6:
                component_field = component_field / norm

        component_field = np.clip(component_field, -1.10, 1.10)
        delta += component_field * influence

    dist_to_mountain = ndimage.distance_transform_edt(~mountain_mask)
    support = (dist_to_mountain <= MOUNTAIN_FOOTHILL_RADIUS) & land_mask
    if support.any():
        support_vals = delta[support]
        norm = float(np.percentile(np.abs(support_vals), 95))
        if norm > 1e-6:
            delta = delta / norm
...
def _build_biome_structure_delta(biomes, land_mask, seed):
    """Create deterministic multi-biome relief variation referenced to mountains."""
    height, width = biomes.shape
    if not land_mask.any():
        return np.zeros((height, width), dtype=np.float32)

    yy, xx = np.indices((height, width), dtype=np.float32)
    rng = np.random.default_rng(seed + 18277)

    mountain_mask = _mountain_mask_from_biomes(biomes, land_mask)
    dist_mountain = ndimage.distance_transform_edt(~mountain_mask)
    mountain_influence = np.exp(-dist_mountain / 9.5) * land_mask.astype(np.float32)

    broad = ndimage.gaussian_filter(rng.normal(0.0, 1.0, size=(height, width)).astype(np.float32), sigma=12.0)
    ...
    wet_mask = land_mask & np.isin(biomes, ['swamp', 'marsh', 'mangrove'])
    snow_mountainish = land_mask & np.isin(biomes, ['glacier', 'snow_mountains', 'alpine_meadows'])

    delta = np.zeros((height, width), dtype=np.float32)

    if plains_mask.any():
        plains_wave = 0.42 * broad + 0.30 * mid
        rainshadow = np.clip((dist_mountain - 5.0) / 22.0, 0.0, 1.0) * mountain_influence
        plains_field = plains_wave - 0.30 * rainshadow
        plains_vals = plains_field[plains_mask]
        norm = float(np.percentile(np.abs(plains_vals), 95)) if plains_vals.size else 0.0
        if norm > 1e-6:
            plains_field /= norm
        delta += np.where(plains_mask, 0.18 * plains_field, 0.0)

    if forest_mask.any():
        forest_roll = 0.35 * broad + 0.36 * mid + 0.17 * fine
        foothill_boost = mountain_influence * (0.18 + 0.14 * np.clip(mid, -1.0, 1.0))
        forest_field = forest_roll + foothill_boost
        forest_vals = forest_field[forest_mask]
        norm = float(np.percentile(np.abs(forest_vals), 94)) if forest_vals.size else 0.0
        if norm > 1e-6:
            forest_field /= norm
        delta += np.where(forest_mask, 0.20 * forest_field, 0.0)

    if desert_mask.any():
        dune_axis = rng.uniform(0.0, np.pi)
        along = np.cos(dune_axis) * (xx - width * 0.5) + np.sin(dune_axis) * (yy - height * 0.5)
        dune_bands = np.sin(along / max(6.0, (width + height) * 0.018))
        desert_field = 0.22 * broad + 0.26 * mid + 0.44 * dune_bands
        desert_field += 0.24 * np.clip(mountain_influence - 0.35, 0.0, 1.0)
        desert_vals = desert_field[desert_mask]
        norm = float(np.percentile(np.abs(desert_vals), 95)) if desert_vals.size else 0.0
        if norm > 1e-6:
            desert_field /= norm
        delta += np.where(desert_mask, 0.22 * desert_field, 0.0)

    if hills_mask.any():
        hill_axis = rng.uniform(0.0, np.pi)
        along = np.cos(hill_axis) * (xx - width * 0.5) + np.sin(hill_axis) * (yy - height * 0.5)
        ghat_step = np.maximum(0.0, np.sin(along / max(4.5, (width + height) * 0.013)))
        hill_field = 0.28 * broad + 0.34 * mid + 0.24 * fine + 0.36 * ghat_step * mountain_influence
        hill_vals = hill_field[hills_mask]
        norm = float(np.percentile(np.abs(hill_vals), 95)) if hill_vals.size else 0.0
        if norm > 1e-6:
            hill_field /= norm
        delta += np.where(hills_mask, 0.26 * hill_field, 0.0)

    if wet_mask.any():
        wet_field = -(0.36 * broad + 0.28 * mid) + 0.14 * fine
        wet_vals = wet_field[wet_mask]
        norm = float(np.percentile(np.abs(wet_vals), 96)) if wet_vals.size else 0.0
        if norm > 1e-6:
            wet_field /= norm
        delta += np.where(wet_mask, 0.22 * wet_field, 0.0)

    if snow_mountainish.any():
        glacial = 0.32 * broad + 0.34 * mid + 0.30 * fine
        glacial -= 0.24 * np.clip(1.0 - mountain_influence * 1.4, 0.0, 1.0)
        cold_vals = glacial[snow_mountainish]
        norm = float(np.percentile(np.abs(cold_vals), 95)) if cold_vals.size else 0.0
        if norm > 1e-6:
            glacial /= norm
        delta += np.where(snow_mountainish, 0.20 * glacial, 0.0)

    support_vals = delta[land_mask]
    if support_vals.size > 0:
        norm = float(np.percentile(np.abs(support_vals), 96))
        if norm > 1e-6:
            delta /= norm
...
def reshape_elevation_by_biome(elevation, biomes, land_mask, is_river, is_lake, seed,
    ...
    Strategy:
    1) Build biome target level/relief maps and blur transitions.
    2) Shift normalized elevation toward target levels with hydro protection.
    3) Scale local relief by biome type (mountains rougher, wetlands smoother).
    4) Clamp deltas and remap land range for smooth grayscale readability.
    """
    _ = seed  # deterministic path currently relies on deterministic inputs only
    ...
    hydro_preserve = np.ones_like(reshaped, dtype=np.float32)
    if is_river.any():
        river_dist = ndimage.distance_transform_edt(~is_river)
        river_damp = 0.35 + 0.65 * np.clip(river_dist / 6.0, 0.0, 1.0)
        hydro_preserve = np.minimum(hydro_preserve, river_damp)
    hydro_preserve[is_river] = 1.0 - ELEVATION_RIVER_CHANNEL_PRESERVE
    if is_lake.any():
    ...
    mountain_delta = _build_mountain_structure_delta(biomes, land_mask, seed)
    if np.any(mountain_delta):
        mountain_shift = strength * MOUNTAIN_RANGE_CARVE_STRENGTH * mountain_delta * hydro_preserve
        mountain_shift[~land_mask] = 0.0
        reshaped = reshaped + mountain_shift

    biome_delta = _build_biome_structure_delta(biomes, land_mask, seed)
    if np.any(biome_delta):
        biome_shift = strength * BIOME_STRUCTURE_CARVE_STRENGTH * biome_delta * hydro_preserve
        biome_shift[~land_mask] = 0.0
        reshaped = reshaped + biome_shift

    local_base = ndimage.gaussian_filter(reshaped, sigma=ELEVATION_RELIEF_BASE_SIGMA)
    local_detail = reshaped - local_base
    local_relief_factor = (1.0 - ELEVATION_RELIEF_BLEND) + ELEVATION_RELIEF_BLEND * relief_scale
    local_relief_factor[~land_mask] = 1.0
    reshaped = local_base + local_detail * local_relief_factor

    # Preserve additional micro-relief over high mountain tops to avoid broad flat caps.
    mountain_mask = _mountain_mask_from_biomes(biomes, land_mask)
    ...
    if mountain_mask.any():
        inside = ndimage.distance_transform_edt(mountain_mask)
        outside = ndimage.distance_transform_edt(~mountain_mask)
        edge_distance = np.minimum(inside, outside)
        edge_zone = (edge_distance <= MOUNTAIN_EDGE_TRANSITION_WIDTH) & land_mask
        edge_weight = np.zeros_like(reshaped, dtype=np.float32)
        edge_weight[edge_zone] = 1.0 - (edge_distance[edge_zone] / MOUNTAIN_EDGE_TRANSITION_WIDTH)
        edge_weight = ndimage.gaussian_filter(edge_weight, sigma=1.0)
        edge_weight = np.clip(edge_weight * MOUNTAIN_EDGE_TRANSITION_BLEND, 0.0, 0.85)
    ...
    reshaped[is_river] = (
        ...
        + (1.0 - ELEVATION_RIVER_CHANNEL_PRESERVE) * reshaped[is_river]
    )
    reshaped[is_lake] = (
        ELEVATION_LAKE_PRESERVE * base[is_lake]
        + (1.0 - ELEVATION_LAKE_PRESERVE) * reshaped[is_lake]
    )

    delta = np.clip(reshaped - base, -ELEVATION_RESHAPE_MAX_DELTA, ELEVATION_RESHAPE_MAX_DELTA)
    reshaped = base + delta

    blurred = ndimage.gaussian_filter(reshaped, sigma=0.9)
    smooth_weight = np.full_like(reshaped, ELEVATION_FINAL_SMOOTH_BLEND, dtype=np.float32)
    if mountain_mask.any():
        # Keep mountain peaks/ridges sharper than lowland terrain.
        smooth_weight[mountain_mask] *= 0.42
    smooth_weight[~land_mask] = 0.0
    reshaped[land_mask] = (
        (1.0 - smooth_weight[land_mask]) * reshaped[land_mask]
        + smooth_weight[land_mask] * blurred[land_mask]
    )

    # Expand land grayscale utilization for smoother visual gradients.
    land_vals = reshaped[land_mask]
    p1 = float(np.percentile(land_vals, 1))
    p99 = float(np.percentile(land_vals, 99))
    span = max(1e-6, p99 - p1)
    normalized = np.clip((land_vals - p1) / span, 0.0, 1.0)
    normalized = np.power(normalized, ELEVATION_LAND_GAMMA)
...
def river_stats_within_tolerance(reference_stats, candidate_stats):
    """Validate that reshaping did not destabilize river generation too far."""
    def ratio_delta(ref_value, new_value):
        if ref_value <= 0:
            return 0.0 if new_value <= 0 else 1.0
        return abs(float(new_value) - float(ref_value)) / float(ref_value)

    tile_delta = ratio_delta(reference_stats.get('total_tiles', 0), candidate_stats.get('total_tiles', 0))
    ocean_delta = ratio_delta(reference_stats.get('rivers_to_ocean', 0), candidate_stats.get('rivers_to_ocean', 0))
    system_delta = ratio_delta(reference_stats.get('num_rivers', 0), candidate_stats.get('num_rivers', 0))

    is_ok = (
        tile_delta <= RIVER_TOLERANCE_TOTAL_TILES
        and ocean_delta <= RIVER_TOLERANCE_OCEAN_REACH
        and system_delta <= RIVER_TOLERANCE_NUM_SYSTEMS
    )
...
def find_river_headwaters(flow_accum, flow_dir, elevation, is_land, is_mountain, width, height, threshold=RIVER_FLOW_THRESHOLD, dist_ocean=None):
    ...
    land_flow = flow_accum[is_land]
    flow_p65 = float(np.percentile(land_flow, 65)) if len(land_flow) > 0 else float(threshold)
    flow_p90 = float(np.percentile(land_flow, 90)) if len(land_flow) > 0 else float(threshold + 1)

    # Natural source viability proxies (used to suppress weak/arid/random overproduction).
    smoothed_elev = ndimage.gaussian_filter(elevation, sigma=1.4)
    local_relief = np.abs(elevation - smoothed_elev)
    relief_p90 = float(np.percentile(local_relief[is_land], 90)) if is_land.any() else 0.08
    if dist_ocean is not None:
        ocean_dist_p90 = float(np.percentile(dist_ocean[is_land], 90)) if is_land.any() else 45.0
    else:
        ocean_dist_p90 = 45.0
        ...
        for x in range(width):
            ...
            if not has_upstream_river:
                ...
                if headwater_type is not None and flow_accum[y, x] >= RIVER_MIN_HEADWATER_FLOW:
                    lat_band = abs((y / max(1, height - 1)) * 2.0 - 1.0)  # 0=mid-lat/equator, 1=poles
                    elev_cool = np.clip(
                        (elev - hills_elev_threshold) /
                        max(1e-6, (very_high_threshold - hills_elev_threshold)),
                        0.0,
                        1.0
                    )
                    temp_proxy = np.clip(0.72 * (1.0 - lat_band) + 0.28 * (1.0 - elev_cool), 0.0, 1.0)

                    if dist_ocean is not None:
                        coastal_moisture = np.exp(-dist_ocean[y, x] / max(8.0, ocean_dist_p90 * 0.45))
                        inland_arid = np.clip(
                            (dist_ocean[y, x] - ocean_dist_p90 * 0.42) /
                            max(1e-6, ocean_dist_p90 * 0.58),
                            0.0,
                            1.0
                        )
                    else:
                        coastal_moisture = 0.42
                        inland_arid = 0.0

                    relief_norm = np.clip(local_relief[y, x] / max(1e-6, relief_p90), 0.0, 1.0)
                    snowmelt_moisture = elev_cool * (1.0 - temp_proxy)
                    moisture_proxy = np.clip(
                        coastal_moisture * 0.46 + relief_norm * 0.32 + snowmelt_moisture * 0.22,
                        0.0,
                        1.0
                    )

                    flow_strength = np.clip(
                        (flow_accum[y, x] - threshold) / max(1e-6, (flow_p90 - threshold)),
                        0.0,
                        1.0
                    )
                    ...
                    type_quality = {
                        'mountain': 1.00,
                        'highland': 0.93,
                        'interior_highland': 0.82,
                        'snow_spring': 0.78,
                        'hills': 0.74,
                        'interior_source': 0.70,
                    }.get(headwater_type, 0.75)

                    dry_hot_penalty = inland_arid * temp_proxy * (1.0 - moisture_proxy)
                    viability = (
                        flow_strength * 0.52 +
                        moisture_proxy * 0.22 +
                        type_quality * 0.16 +
                        relief_norm * 0.10 -
                        dry_hot_penalty * 0.28
                    )
                    if flow_accum[y, x] >= flow_p65 and headwater_type in ('mountain', 'highland'):
                        viability += 0.04
                    if flow_accum[y, x] >= flow_p90 and headwater_type in ('mountain', 'highland'):
                        viability += 0.06
                    viability = float(np.clip(viability, 0.0, 1.0))

                    keep_prob = float(np.clip((viability - 0.32) / 0.55, 0.05, 0.98))
                    keep_noise = _coord_noise_01(y, x, seed=int(flow_accum[y, x] * 11), salt=733)
                    if keep_noise <= keep_prob:
    ...
    if dist_ocean is not None:
        ...
        for y in range(height):
            for x in range(width):
                ...
                if path_length >= min_path_length and is_land[ty, tx] == False:
                    lat_band = abs((y / max(1, height - 1)) * 2.0 - 1.0)
                    elev_cool = np.clip(
                        (elev - hills_elev_threshold) /
                        max(1e-6, (very_high_threshold - hills_elev_threshold)),
                        0.0,
                    ...
                    snowmelt_moisture = elev_cool * (1.0 - temp_proxy)
                    moisture_proxy = np.clip(
                        coastal_moisture * 0.42 + relief_norm * 0.36 + snowmelt_moisture * 0.22,
                        0.0,
                        1.0
                    )
                    flow_strength = np.clip(
                    ...
                    interior_arid = np.clip(
                        (dist_ocean[y, x] - ocean_dist_p90 * 0.45) /
                        max(1e-6, ocean_dist_p90 * 0.55),
                        0.0,
                        1.0
                    )
                    source_viability = (
                        ...
                        moisture_proxy * 0.30 +
                        relief_norm * 0.24 -
                        (temp_proxy * interior_arid) * 0.22
                    )
                    if source_viability < 0.28:
                        continue

                    score = (
                        path_length * (0.80 + flow_strength * 0.60) +
                        dist_ocean[y, x] * 0.35 +
                        source_viability * 28.0
                    )
                    interior_candidates.append((y, x, score, path_length, dist_ocean[y, x], elev, source_viability))
        ...
        target_interior = max(3, min(8, interior_land_tiles // 7000 + 3))

        # Rare seed/map configurations can still become river-rich.
        interior_roll = _coord_noise_01(
            int(np.mean(land_elevations) * 10000),
            int(np.std(land_elevations) * 10000),
            seed=width * 8191 + height * 131,
            salt=271
        )
        if interior_roll > 0.92:
            target_interior += 3
        
        for y, x, score, path_len, d_ocean, elev, source_viability in interior_candidates:
            if added_interior >= target_interior:
                break
            if (y, x) in existing_headwater_positions:
                continue

            keep_prob = float(np.clip((source_viability - 0.24) / 0.62, 0.08, 0.95))
            keep_noise = _coord_noise_01(y, x, seed=int(score * 10), salt=401)
            if keep_noise > keep_prob:
                continue
...
def create_river_from_accumulation(flow_accum, flow_dir, elevation, is_land, is_mountain, width, height, threshold=RIVER_FLOW_THRESHOLD, dist_ocean=None, seed=None, is_lake=None):
    ...
    density_rng = np.random.default_rng((seed or 0) + 9999)
    density_roll = density_rng.random()
    if density_roll < 0.15:
        # Rare river-rich seeds keep the old "many rivers" feel.
        target_max_rivers = TARGET_MAX_RIVERS + int(density_rng.integers(4, 10))
    else:
        # Default worlds are less river-dense.
        target_max_rivers = max(18, int(round(TARGET_MAX_RIVERS * 0.86)))
    ...
    rng_inland = np.random.default_rng((seed or 0) + 19999)
    if rng_inland.random() < INLAND_RIVER_CHANCE:
        target_inland_rivers = int(rng_inland.integers(1, 4))  # 1-3 inland rivers
    else:
        target_inland_rivers = 1  # Always allow at least 1 inland river
    interior_land_tiles = 0
    if dist_ocean is not None:
        interior_land_tiles = int((is_land & (dist_ocean >= INTERIOR_RIVER_DIST_THRESHOLD)).sum())
        interior_target = max(1, min(5, interior_land_tiles // 7000))
        target_inland_rivers = max(target_inland_rivers, interior_target)

    if density_roll < 0.15:
        target_inland_rivers += 1
...
def generate_world(seed):
    ...
    total_steps = 17 if ENABLE_BIOME_ELEVATION_RESHAPING else 16
    ...
    step_header(15, "Assigning provisional biomes with expanded classification system..." if ENABLE_BIOME_ELEVATION_RESHAPING else "Assigning biomes with expanded classification system...")
    ...
    if ENABLE_BIOME_ELEVATION_RESHAPING:
        step_header(16, "Reshaping elevation by biome and recomputing hydro-climate...")
        land_mask = is_land | is_island
        baseline_elevation = elevation.copy()
        baseline_river_stats = dict(river_stats)
        ...
        is_river, river_ids, river_map, river_width, river_stats, elevation, river_hierarchy, headwater_types, river_moisture_intensity, river_paths = generate_rivers(
            width, height, seed, elevation, is_land, is_mountain, is_lake, lake_ids, dist_ocean, return_paths=True
        )
        river_ok, river_delta = river_stats_within_tolerance(baseline_river_stats, river_stats)
        print(
            "  - River stability deltas:"
            f" tiles={river_delta['tile_delta']:.2f},"
            f" ocean_reach={river_delta['ocean_delta']:.2f},"
            f" systems={river_delta['system_delta']:.2f}"
        )

        if not river_ok and ENABLE_RIVER_RESHAPE_FALLBACK:
            retry_strength = ELEVATION_RESHAPE_BASE_STRENGTH * ELEVATION_RESHAPE_RETRY_FACTOR
            print(f"  - River drift above tolerance; retrying reshape with reduced strength ({retry_strength:.3f})")
        ...
        # Ensure only ocean-reachable river fragments remain with islands in final land mask.
        post_island_reach = compute_ocean_reachability(is_river, is_land_with_islands, width, height)
        prune_mask = is_river & is_land_with_islands & ~post_island_reach
        if prune_mask.any():
        ...
        # Final biome assignment from reshaped terrain and recomputed hydro-climate.
        biomes = assign_biomes_expanded(
            width, height, seed, is_land, is_mountain, is_lake, is_river,
            temperature, moisture, elevation, slope, roughness,
            dist_river, dist_coast, is_island, dist_ocean=dist_ocean
        ...
        biomes = enforce_biome_categories(
            biomes, land_mask, temperature, moisture, elevation,
            dist_river, dist_lake, dist_coast, seed
        )
        ...
        biomes = enforce_expanded_biome_presence(
            biomes, land_mask, post_landforms, temperature, moisture, elevation, slope, roughness,
            dist_river, dist_coast, dist_ocean, post_dist_mountain, post_dist_hills,
            post_thresholds, is_lake, is_river, seed + 23
        )

        # Final post-reshape biome distribution summary.
        biome_counts = {}
        for y in range(height):
            for x in range(width):
                if land_mask[y, x] and not is_lake[y, x]:
                    biome_name = biomes[y, x]
                    biome_counts[biome_name] = biome_counts.get(biome_name, 0) + 1

        total_land_biomes = max(1, sum(biome_counts.values()))
        print(f"  - Final expanded biome types used: {len(biome_counts)}")
        top_final = sorted(biome_counts.items(), key=lambda kv: kv[1], reverse=True)[:8]
        for biome_name, count in top_final:
            pct = count / total_land_biomes * 100
            print(f"    {biome_name}: {count} ({pct:.1f}%)")

        missing_final = [b for b in EXPANDED_REQUIRED_BIOMES if biome_counts.get(b, 0) == 0]
        if missing_final:
            print(f"  - Missing expanded biomes after reshape pass: {missing_final}")
        else:
            print("  - All expanded biomes present after reshape pass")
    
    resource_step = 17 if ENABLE_BIOME_ELEVATION_RESHAPING else 16
    step_header(resource_step, "Computing tile environment resources...")
...
def generate_atlas_map(df, seed, biome_colors, save_image=True):
    ...
    atlas_img = np.zeros((height, width, 3), dtype=np.float32)
    atlas_colors = np.array([color_to_rgb01(biome_colors.get(b, '#000000')) for b in biome_vals], dtype=np.float32)
    atlas_colors[river_vals] = color_to_rgb01('#1e90ff')
    atlas_img[y_vals, x_vals] = atlas_colors

    if save_image:
        output_dir = os.path.join("map_layers", str(seed))
        os.makedirs(output_dir, exist_ok=True)
        atlas_uint8 = np.clip(atlas_img * 255.0, 0, 255).astype(np.uint8)
        atlas_pil = Image.fromarray(atlas_uint8, mode='RGB')
        if atlas_pil.size != (1024, 1024):
            resampling = Image.Resampling.NEAREST if hasattr(Image, "Resampling") else Image.NEAREST
            atlas_pil = atlas_pil.resize((1024, 1024), resample=resampling)
        atlas_pil.save(os.path.join(output_dir, 'atlas.png'))
