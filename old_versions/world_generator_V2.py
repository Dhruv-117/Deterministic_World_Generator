...
LAKE_PERCENT_MIN = 0.05  # Min 5% of land for lakes
LAKE_PERCENT_MAX = 0.09  # Max 9% of land for lakes
...
MIN_LAKE_SPACING = 10  # Minimum distance between lake centers (spread them out)
...
def generate_lakes(width, height, seed, elevation, is_land, is_mountain, distance_from_ocean):
    ...
    max_lakes = 80  # More lakes allowed since they're smaller
    start_idx = 0
...
def generate_islands(width, height, seed, is_land, elevation):
    ...
    if roll < 0.40:
        ...
        while placed < target_islands and i < len(ocean_tiles):
            ...
            island_size = rng.integers(2, 6)  # Minimum 2 tiles
            island_tiles = grow_island(x, y, island_size, is_land, is_island, width, height)
            
            if len(island_tiles) >= 2:  # No single-tile islands
...
def grow_island(start_x, start_y, target_size, is_land, is_island, width, height):
    """Grow an island."""
    island_tiles = [(start_x, start_y)]
    frontier = [(start_x, start_y)]
    visited = {(start_x, start_y)}
    
    while len(island_tiles) < target_size and frontier:
        ...
        for dy, dx in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            ...
            if 0 <= nx < width and 0 <= ny < height:
                if (nx, ny) not in visited:
                    if not is_land[ny, nx] and not is_island[ny, nx]:
                        visited.add((nx, ny))
                        island_tiles.append((nx, ny))
                        frontier.append((nx, ny))
                        if len(island_tiles) >= target_size:
                            break
...
def assign_regions(width, height):
    ...
    region_x = np.zeros((height, width), dtype=np.int32)
    region_y = np.zeros((height, width), dtype=np.int32)
    
    for y in range(height):
        for x in range(width):
            region_x[y, x] = x // REGION_SIZE
            region_y[y, x] = y // REGION_SIZE
    ...
    for octave in range(octaves):
        ...
        for y in range(height):
            for x in range(width):
                ...
                nx = (x / scale * freq + phase_x)
                ny = (y / scale * freq + phase_y)
                noise[y, x] += amp * (np.sin(nx) * np.cos(ny) + 1) / 2
...
def generate_world(seed):
    ...
    print("\n[1/13] Generating tectonic plates...")
    ...
    print("\n[5/13] Identifying mountains (10% of land)...")
    ...
    print("\n[6/13] Computing distance from ocean...")
    ...
    print("\n[7/14] Generating realistic river network...")
    ...
    print("\n[8/14] Generating lakes (targeting 5-9% of land)...")
    ...
    print("\n[9/14] Computing distance fields for rivers and lakes...")
    ...
    print("\n[9.5/14] Computing derived terrain maps (slope, roughness, coast distance)...")
    ...
    print("\n[10/14] Generating natural temperature simulation...")
    ...
    print("\n[11/14] Generating natural moisture simulation (with wind & rain shadow)...")
    ...
    print("\n[12/14] Applying river climate effects...")
    ...
    print("\n[14/14] Assigning biomes with expanded classification system...")
    ...
    print("\n[15/15] Computing tile environment resources...")
    ...
    for y in range(height):
        for x in range(width):
            tile = {
                'x': x,
                'y': y,
                'region_id_x': region_x[y, x],
                'region_id_y': region_y[y, x],
                'plate_id': plate_map[y, x],
                'elevation': round(elevation[y, x], 6),
                'temperature': round(temperature[y, x], 6),
                'moisture': round(moisture[y, x], 6),
                'dist_ocean': round(dist_ocean[y, x], 2),
                'dist_river': round(dist_river[y, x], 2),
                'dist_lake': round(dist_lake[y, x], 2),
                'biome': biomes[y, x],
                'is_land': bool(is_land_with_islands[y, x]),
                'is_mountain': bool(is_mountain[y, x]),
                'is_river': bool(is_river[y, x]),
                'is_lake': bool(is_lake[y, x]),
                'is_headwater': (y, x) in headwater_types,
                'river_id': int(river_ids[y, x]),
                'lake_id': int(lake_ids[y, x]),
            }
            
            # Add environment resource data
            tile.update(get_tile_environment_data(tile_env, y, x))
...
def visualize_world(df, seed, save_image=True):
    ...
    for _, row in df.iterrows():
        ...
        biome = row['biome']
        color = mcolors.to_rgb(biome_colors.get(biome, '#000000'))
        
        # Adjust for elevation
        elev = row['elevation']
        brightness = 0.7 + 0.3 * elev
        color = tuple(min(1.0, c * brightness) for c in color)
        
        # Mark rivers in blue
        if row['is_river']:
            color = mcolors.to_rgb('#1e90ff')  # Dodger blue
    ...
    atlas_img = np.zeros((height, width, 3))
    for _, row in df.iterrows():
        x, y = int(row['x']), int(row['y'])
        biome = row['biome']
        color = mcolors.to_rgb(biome_colors.get(biome, '#000000'))
        ...
        if row['is_river']:
            color = mcolors.to_rgb('#1e90ff')  # Dodger blue rivers
    ...
    moisture_img = np.zeros((height, width))
    for _, row in df.iterrows():
        x, y = int(row['x']), int(row['y'])
        moisture_img[y, x] = row['moisture']
...
def visualize_resources(df, seed, save_image=True):
    ...
    # Populate arrays from dataframe
    for _, row in df.iterrows():
        x, y = int(row['x']), int(row['y'])
        
        # Check if water tile
        biome = row.get('biome', '')
        if biome in ['ocean', 'lake'] or row.get('is_river', False):
            is_water[y, x] = True
        
        # Metal ores: weighted combination (iron common, gold rare)
        metal_ores[y, x] = (
            row.get('iron_potential', 0) * 0.3 +
            row.get('copper_potential', 0) * 0.3 +
            row.get('tin_potential', 0) * 0.2 +
            row.get('gold_potential', 0) * 0.2
        )
        
        # Coal and fossil fuels
        coal_fossil[y, x] = (
            row.get('coal_potential', 0) * 0.5 +
            row.get('fossil_fuel_potential', 0) * 0.3 +
            row.get('peat_potential', 0) * 0.2
        )
        
        # Wood biomass
        wood_biomass[y, x] = row.get('wood_biomass_capacity', 0)
        
        # Water resources (fresh water + groundwater, excluding ocean/lake tiles)
        if not is_water[y, x]:
            water_resources[y, x] = (
                row.get('fresh_water', 0) * 0.6 +
                row.get('groundwater_potential', 0) * 0.4
            )
        
        # Soil fertility
        soil_fertility[y, x] = row.get('soil_fertility', 0)
        
        # Overall resource richness (normalized sum)
        overall_richness[y, x] = (
            metal_ores[y, x] * 0.2 +
            coal_fossil[y, x] * 0.15 +
            wood_biomass[y, x] * 0.2 +
            row.get('fresh_water', 0) * 0.15 +
            soil_fertility[y, x] * 0.15 +
            row.get('mineral_potential', 0) * 0.1 +
            row.get('animal_habitat_capacity', 0) * 0.05
        )
...
if __name__ == "__main__":
    ...
    # Default seed or from command line
    if len(sys.argv) > 1:
        try:
            seed = int(sys.argv[1])
        except ValueError:
    ...
    if visualize_choice == 'y':
        ...
        # Resource map
        resource_choice = input("Generate resource map? (y/n): ").strip().lower()
        if resource_choice == 'y':
